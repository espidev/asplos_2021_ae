...
__device__ void Body_apply_force(IndexT id, IndexT other) {
  ...
  if (other != id) {
    ...
    float dist = d_bodies[other]->computeDistance(d_bodies[id]);

    float F = d_bodies[other]->computeForce(d_bodies[id], dist);
...
template <void (*func)(IndexT)> __global__ void parallel_do() {
  int tid = threadIdx.x + blockDim.x * blockIdx.x;
  for (int id = tid; id < kNumBodies; id += blockDim.x * gridDim.x) {
...
void transfer_data() {
  ...
  cudaMemcpyFromSymbol(host_Body_vel_y, r_Body_vel_y,
                       sizeof(float) * kNumBodies, 0, cudaMemcpyDeviceToHost);
  cudaMemcpyFromSymbol(host_Body_mass, r_Body_mass, sizeof(float) * kNumBodies,

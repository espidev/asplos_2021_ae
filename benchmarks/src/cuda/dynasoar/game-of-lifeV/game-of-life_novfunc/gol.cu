  ...
  for (int dx = -1; dx < 2; ++dx) {
    for (int dy = -1; dy < 2; ++dy) {
      ...
      if (nx > -1 && nx < SIZE_X && ny > -1 && ny < SIZE_Y) {
        Agent *tmp = cells[ny * SIZE_X + nx]->agent();
...
__device__ void maybe_create_candidate(Agent *ptr, int x, int y) {
  ...
  for (int dx = -1; dx < 2; ++dx) {
    for (int dy = -1; dy < 2; ++dy) {
      ...
      if (nx > -1 && nx < SIZE_X && ny > -1 && ny < SIZE_Y) {
        Agent *alive = cells[ny * SIZE_X + nx]->agent();
        if (alive != nullptr) {
          if (alive->is_new()) {
            if (alive == ptr) {
...
__device__ void create_candidates(Agent *ptr) {
  assert(ptr->is_new());
  assert(ptr->isAlive());
...
__device__ void Alive_prepare(Agent *ptr) {
  if (ptr) {
    if (ptr->isAlive()) {
...
__device__ void Alive_update(Agent *ptr) {
  if (ptr) {
    if (ptr->isAlive()) {
      int cid = ptr->cell_id();
...
__device__ void Candidate_prepare(Agent *ptr) {
  if (ptr) {
    if (ptr->isCandidate()) {
...
__device__ void Candidate_update(Agent *ptr) {
...
ALL void Agent::update_checksum() {
  if (this->isAlive())

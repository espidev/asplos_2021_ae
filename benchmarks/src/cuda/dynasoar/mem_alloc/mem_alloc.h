...
public:
  ...
  template <class myType> void *my_new() {
    ...
    if (type_map.find(x) == type_map.end()) {
      ...
      bucket = new range_bucket(CALLOC_NUM, sizeof(myType),
                                mem->calloc<myType>(CALLOC_NUM), gpu_vtable,
                                cpu_vtable);
      num_of_ranges++;
  ...
  void type_to_device(list<range_bucket *> *list_ptr) {
    ...
    for (iter = list_ptr->begin(); iter != list_ptr->end(); ++iter) {
       vptrPatch<<<grid,threads>>>((*iter)->mem_ptr,(*iter)->gpu_vtable,(*iter)->type_size,CALLOC_NUM);
           cudaDeviceSynchronize();
    ...
    dim3 grid(num_blocks, 1, 1);
    for (iter = list_ptr->begin(); iter != list_ptr->end(); ++iter) {
       vptrPatch<<<grid,threads>>>((*iter)->mem_ptr,(*iter)->cpu_vtable,(*iter)->type_size,CALLOC_NUM);
           cudaDeviceSynchronize();

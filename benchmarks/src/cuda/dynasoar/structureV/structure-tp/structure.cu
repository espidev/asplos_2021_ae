...
__device__ void new_NodeBase(NodeBase *node, float pos_x, float pos_y) {
    CLEANPTR(node, NodeBase *)->pos_x = pos_x;
    CLEANPTR(node, NodeBase *)->pos_y = pos_y;
    CLEANPTR(node, NodeBase *)->num_springs = 0;
    CLEANPTR(node, NodeBase *)->type = kTypeNodeBase;

    for (int i = 0; i < kMaxDegree; ++i) {
        CLEANPTR(node, NodeBase *)->springs[i] = NULL;
    }
}

__device__ void new_AnchorNode(NodeBase *node, float pos_x, float pos_y) {
    new_NodeBase(node, pos_x, pos_y);
    CLEANPTR(node, NodeBase *)->type = kTypeAnchorNode;
}

__device__ void new_AnchorPullNode(NodeBase *node, float pos_x, float pos_y,
                                   float vel_x, float vel_y) {
    new_AnchorNode(node, pos_x, pos_y);
    CLEANPTR(node, NodeBase *)->vel_x = vel_x;
    CLEANPTR(node, NodeBase *)->vel_y = vel_y;
    CLEANPTR(node, NodeBase *)->type = kTypeAnchorPullNode;
}

__device__ void new_Node(NodeBase *node, float pos_x, float pos_y, float mass) {
    new_NodeBase(node, pos_x, pos_y);
    CLEANPTR(node, NodeBase *)->mass = mass;
    CLEANPTR(node, NodeBase *)->type = kTypeNode;
}
...
__device__ void NodeBase_add_spring(NodeBase *node, SpringBase *spring) {
    ...
    int idx = atomicAdd(&CLEANPTR(node, NodeBase *)->num_springs, 1);
    assert(idx + 1 <= kMaxDegree);
    CLEANPTR(node, NodeBase *)->springs[idx] = spring;
...
__device__ void new_Spring(SpringBase *spring, NodeBase *p1, NodeBase *p2,
    ...
    CLEANPTR(spring, SpringBase *)->is_active = true;
    CLEANPTR(spring, SpringBase *)->p1 = p1;
    CLEANPTR(spring, SpringBase *)->p2 = p2;
    CLEANPTR(spring, SpringBase *)->factor = spring_factor;
    CLEANPTR(spring, SpringBase *)->force = 0.0f;
    CLEANPTR(spring, SpringBase *)->max_force = max_force;
    CLEANPTR(spring, SpringBase *)->initial_length =
        CLEANPTR(p1, NodeBase *)->distance_to(p2);
    CLEANPTR(spring, SpringBase *)->delete_flag = false;
    // if (!(spring->initial_length > 0.0f))
    //   printf("%f \n", spring->initial_length);
    assert(CLEANPTR(spring, SpringBase *)->initial_length > 0.0f);
...
__device__ void NodeBase_remove_spring(NodeBase *node, SpringBase *spring) {
    for (int i = 0; i < kMaxDegree; ++i) {
        if (CLEANPTR(node, NodeBase *)->springs[i] == spring) {
            CLEANPTR(node, NodeBase *)->springs[i] = NULL;
            if (atomicSub(&CLEANPTR(node, NodeBase *)->num_springs, 1) == 1) {
                // Deleted last spring.
                CLEANPTR(node, NodeBase *)->type = 0;
            }
            return;
        }
    }
...
__device__ void AnchorPullNode_pull(NodeBase *node) {
    CLEANPTR(node, NodeBase *)->pos_x +=
        CLEANPTR(node, NodeBase *)->vel_x * kDt;
    CLEANPTR(node, NodeBase *)->pos_y +=
        CLEANPTR(node, NodeBase *)->vel_y * kDt;
}

__device__ void Spring_self_destruct(SpringBase *spring) {
    NodeBase_remove_spring(CLEANPTR(spring,SpringBase*)->get_p1(), spring);
    NodeBase_remove_spring(CLEANPTR(spring,SpringBase*)->get_p2(), spring);
    CLEANPTR(spring, SpringBase *)->is_active = false;
}

__device__ void Spring_compute_force(SpringBase *spring) {
    void **vtable;
    COAL_SpringBase_get_p2(spring) NodeBase *p2 =
        CLEANPTR(spring, SpringBase *)->get_p2();
    COAL_SpringBase_get_p1(spring) NodeBase *p1 =
        CLEANPTR(spring, SpringBase *)->get_p1();

    COAL_NodeBase_distance_to(p1);
    float dist = CLEANPTR(p1, NodeBase *)->distance_to(p2);
    COAL_SpringBase_get_init_len(spring);
    float displacement =
        max(0.0f, dist - CLEANPTR(spring, SpringBase *)->get_init_len());
    COAL_SpringBase_update_force(spring);
    CLEANPTR(spring, SpringBase *)->update_force(displacement);
    COAL_SpringBase_is_max_force(spring);
    if (CLEANPTR(spring, SpringBase *)->is_max_force()) {
        COAL_NodeBase_remove_spring(p1);
        CLEANPTR(p1, NodeBase *)->remove_spring(spring);
        COAL_NodeBase_remove_spring(p2);
        CLEANPTR(p2, NodeBase *)->remove_spring(spring);
        COAL_SpringBase_deactivate(spring);
        CLEANPTR(spring, SpringBase *)->deactivate();
...
__device__ void Node_move(NodeBase *node) {
    ...
    for (int i = 0; i < kMaxDegree; ++i) {
        ...
        SpringBase *s = CLEANPTR(node, NodeBase *)->spring(i);

        if (s != NULL) {
            NodeBase *from;
            NodeBase *to;
            COAL_SpringBase_get_p1(s);
            if (CLEANPTR(s, SpringBase *)->get_p1() == node) {
                from = node;
                COAL_SpringBase_get_p2(s);
                to = CLEANPTR(s, SpringBase *)->get_p2();
            } else {
                COAL_SpringBase_get_p2(s);
                assert(CLEANPTR(s, SpringBase *)->get_p2() == node);
                from = node;
                COAL_SpringBase_get_p1(s);
                to = CLEANPTR(s, SpringBase *)->get_p1();
            }

            // Calculate unit vector.
            COAL_NodeBase_distance_to(to);
            float dist = CLEANPTR(to, NodeBase *)->distance_to(from);
            COAL_NodeBase_unit_x(to);
            float unit_x = CLEANPTR(to, NodeBase *)->unit_x(from, dist);
            COAL_NodeBase_unit_y(to);
            float unit_y = CLEANPTR(to, NodeBase *)->unit_y(from, dist);

            // Apply force.
            COAL_SpringBase_get_force(s);
            force_x += unit_x * CLEANPTR(s, SpringBase *)->get_force();
            COAL_SpringBase_get_force(s);
            force_y += unit_y * CLEANPTR(s, SpringBase *)->get_force();
        }
    }

    // Calculate new velocity and position.
    COAL_NodeBase_update_vel_x(node) CLEANPTR(node, NodeBase *)
        ->update_vel_x(force_x);
    COAL_NodeBase_update_vel_y(node) CLEANPTR(node, NodeBase *)
        ->update_vel_y(force_y);
    COAL_NodeBase_update_pos_x(node) CLEANPTR(node, NodeBase *)
        ->update_pos_x(force_x);
    COAL_NodeBase_update_pos_y(node) CLEANPTR(node, NodeBase *)
        ->update_pos_y(force_y);
}

__device__ void NodeBase_initialize_bfs(NodeBase *node) {
    void **vtable;
    if (CLEANPTR(node, NodeBase *)->type == kTypeAnchorNode) {
        COAL_NodeBase_set_distance(node) CLEANPTR(node, NodeBase *)
            ->set_distance(0);
    } else {
        COAL_NodeBase_set_distance(node) CLEANPTR(node, NodeBase *)
            ->set_distance(kMaxDistance);  // should be int_max
    }
}
...
__device__ void NodeBase_bfs_visit(NodeBase *node, int distance) {
    void **vtable;
    COAL_NodeBase_get_distance(
        node); if (distance == CLEANPTR(node, NodeBase *)->get_distance()) {
        // Continue until all vertices were visited.
        dev_bfs_continue = true;

        for (int i = 0; i < kMaxDegree; ++i) {
            COAL_NodeBase_spring(node) SpringBase *spring =
                CLEANPTR(node, NodeBase *)->spring(i);

            if (spring != NULL) {
                // Find neighboring vertices.
                NodeBase *n;
                COAL_SpringBase_get_p1(spring);
                if (node == CLEANPTR(spring, SpringBase *)->get_p1()) {
                    COAL_SpringBase_get_p2(spring);
                    n = CLEANPTR(spring, SpringBase *)->get_p2();
                } else {
                    COAL_SpringBase_get_p1(spring);
                    n = CLEANPTR(spring, SpringBase *)->get_p1();
                }
                COAL_NodeBase_get_distance(n);
                if (CLEANPTR(n,NodeBase*)->get_distance() == kMaxDistance) {
                    // Set distance on neighboring vertex if unvisited.
                    COAL_NodeBase_set_distance(n);
                    CLEANPTR(n,NodeBase*)->set_distance(distance + 1);
                }
            }
        }
    }
...
__device__ void Spring_bfs_delete(SpringBase *spring) {
    void **vtable;
    if (CLEANPTR(spring, SpringBase *)->delete_flag) {
        COAL_SpringBase_get_p1(spring) NodeBase *p1 =
            CLEANPTR(spring, SpringBase *)->get_p1();
        COAL_SpringBase_get_p2(spring) NodeBase *p2 =
            CLEANPTR(spring, SpringBase *)->get_p2();
        COAL_NodeBase_remove_spring(p1) CLEANPTR(p1, NodeBase *)
            ->remove_spring(spring);
        COAL_NodeBase_remove_spring(p2) CLEANPTR(p2, NodeBase *)
            ->remove_spring(spring);
        COAL_SpringBase_deactivate(spring) CLEANPTR(spring, SpringBase *)
            ->deactivate();
    }
}
...
__device__ void NodeBase_bfs_set_delete_flags(NodeBase *node) {
    void **vtable;
    if (CLEANPTR(node, NodeBase *)->distance ==
        kMaxDistance) {  // should be int_max
        for (int i = 0; i < kMaxDegree; ++i) {
            COAL_NodeBase_spring(node) SpringBase *spring =
                CLEANPTR(node, NodeBase *)->spring(i);
            if (spring != NULL) {
                CLEANPTR(spring, SpringBase *)->delete_flag = true;
...
__device__ void Spring_add_to_rendering_array(SpringBase *spring) {
    ...
    dev_spring_info[idx].p1_x = CLEANPTR(CLEANPTR(spring,SpringBase*)->p1,NodeBase *)->pos_x;
    dev_spring_info[idx].p1_y = CLEANPTR(CLEANPTR(spring,SpringBase*)->p1,NodeBase *)->pos_y;
    dev_spring_info[idx].p2_x = CLEANPTR(CLEANPTR(spring,SpringBase*)->p2,NodeBase *)->pos_x;
    dev_spring_info[idx].p2_y = CLEANPTR(CLEANPTR(spring,SpringBase*)->p2,NodeBase *)->pos_y;
    dev_spring_info[idx].force = CLEANPTR(spring,SpringBase*)->force;
    dev_spring_info[idx].max_force = CLEANPTR(spring,SpringBase*)->max_force;
}
...
__global__ void kernel_AnchorPullNode_pull() {
    for (int i = threadIdx.x + blockDim.x * blockIdx.x; i < kMaxNodes;
        ...
        void **vtable;
        if (CLEANPTR(dev_nodes[i], NodeBase *)->type == kTypeAnchorPullNode) {
            COAL_NodeBase_pull(dev_nodes[i]);
            CLEANPTR(dev_nodes[i], NodeBase *)->pull();
        }
    }
}
...
__global__ void kernel_Node_move() {
    for (int i = threadIdx.x + blockDim.x * blockIdx.x; i < kMaxNodes;
         i += blockDim.x * gridDim.x) {
        if (CLEANPTR(dev_nodes[i], NodeBase *)->type == kTypeNode) {
...
__global__ void kernel_NodeBase_initialize_bfs() {
    for (int i = threadIdx.x + blockDim.x * blockIdx.x; i < kMaxNodes;
         i += blockDim.x * gridDim.x) {
        if (CLEANPTR(dev_nodes[i], NodeBase *)->type != 0) {
...
__global__ void kernel_Spring_compute_force() {
    ...
    for (int i = threadIdx.x + blockDim.x * blockIdx.x; i < kMaxSprings;
        ...
        if (CLEANPTR(ptr, SpringBase *)->get_is_active()) {
...
__global__ void kernel_Spring_bfs_delete() {
    for (int i = threadIdx.x + blockDim.x * blockIdx.x; i < kMaxSprings;
         i += blockDim.x * gridDim.x) {
        if (CLEANPTR(dev_springs[i], SpringBase *)->get_is_active()) {
...
void initialize_nodes(obj_alloc *alloc) {
    for (int i = 0; i < kMaxNodes; i += 1) {
        ...
        CLEANPTR(dev_nodes[i], NodeBase *)->type = 0;
    }
}
...
void initialize_springs(obj_alloc *alloc) {
    for (int i = 0; i < kMaxSprings; i += 1) {
        ...
        CLEANPTR(dev_springs[i], SpringBase *)->is_active = false;
    }
}

...
__device__ void Car_step_extend_path(IndexT self) {
    void **vtable;
    COAL_Car_get_position(dev_cars[self]);
...
__device__ void Car_step_constraint_velocity(IndexT self) {
    ...
    while (distance <= dev_cars[self]->get_velocity()) {
        ...
        COAL_Car_get_path(dev_cars[self]);
        ...
        COAL_Cell_get_current_max_velocity(next_cell);
    ...
    for (int i = 0; i < dev_cars[self]->get_velocity(); ++i) {
        COAL_Car_get_path(dev_cars[self]);
        CellBase *path = dev_cars[self]->get_path(i);
        COAL_Cell_is_free(path);
        assert(path->is_free());
        COAL_Car_get_path(dev_cars[self]);
        assert(i == 0 || dev_cars[self]->get_path(i - 1) != path);
    }
...
__device__ void Car_step_move(IndexT self) {
    ...
    for (int i = 0; i < dev_cars[self]->get_velocity(); ++i) {
        ...
        COAL_Cell_is_free(cell);
        assert(cell->is_free());
        COAL_Car_get_position(dev_cars[self]);
        CellBase *ptr = dev_cars[self]->get_position();
        COAL_Cell_release(ptr);
        ptr->release();
        COAL_Cell_occupy(cell);
        cell->occupy(dev_cars[self]);
        COAL_Car_set_position(dev_cars[self]);
        dev_cars[self]->set_position(cell);
    }
    COAL_Car_get_position(dev_cars[self]);
    CellBase *ptr = dev_cars[self]->get_position();
    COAL_Cell_is_sink(ptr);
    bool cond = ptr->is_sink();
    COAL_Cell_get_is_target(ptr);
...
__device__ IndexT new_Car(int seed, IndexT cell, int max_velocity) {
    ...
    COAL_Car_set_position(ptr);
    ptr->set_position(dev_cells[cell]);
    COAL_Car_set_path_length(ptr);
    ptr->set_path_length(0);

    COAL_Car_set_velocity(ptr);

    ptr->set_velocity(0);
    COAL_Car_set_max_velocity(ptr);
    ptr->set_max_velocity(max_velocity);
    d_Car_active[idx] = 1;

    COAL_Cell_is_free(dev_cells[cell]);
    assert(dev_cells[cell]->is_free());
    COAL_Cell_occupy(dev_cells[cell]);
    dev_cells[cell]->occupy(dev_cars[idx]);
    curand_init(seed, 0, 0, &ptr->random_state);
...
__device__ void ProducerCell_create_car(IndexT self) {
    ...
    COAL_Cell_is_free(dev_cells[self]);
...
__global__ void kernel_traffic_light_step() {
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < kNumIntersections;
        ...
        COAL_Traffic_get_num_cells(ptr);
        if (ptr->get_num_cells() > 0) {
            COAL_Traffic_get_timer(ptr);
            int timer = ptr->get_timer();
            COAL_Traffic_get_phase_time(ptr);
            int phase_time = ptr->get_phase_time();
            COAL_Traffic_set_timer(ptr);
            ptr->set_timer((timer + 1) % phase_time);
            COAL_Traffic_get_timer(ptr);
            if (ptr->get_timer() == 0) {
                COAL_Traffic_get_phase(ptr);
                int phase = ptr->get_phase();
                COAL_Traffic_get_cell(ptr);
                assert(ptr->get_cell(phase) != nullptr);
                COAL_Traffic_get_phase(ptr);
                phase = ptr->get_phase();
                COAL_Traffic_get_cell(ptr);
                CellBase *ptr2 = ptr->get_cell(phase);
                COAL_Cell_set_current_max_velocity(ptr2);
                ptr2->set_current_max_velocity(0);
                COAL_Traffic_get_phase(ptr);
                int phase_2 = ptr->get_phase();
                COAL_Traffic_get_num_cells(ptr);
                int num_cells = ptr->get_num_cells();
                COAL_Traffic_set_phase(ptr);
                ptr->set_phase((phase_2 + 1) % num_cells);
                COAL_Traffic_get_phase(ptr);
                phase_2 = ptr->get_phase();
                COAL_Traffic_get_cell(ptr);
                ptr2 = ptr->get_cell(phase_2);
                COAL_Cell_remove_speed_limit(ptr2);
                ptr2->remove_speed_limit();
            }
        }
...
__device__ void Car_step_prepare_path(IndexT self) {
    void **vtable;
    COAL_Car_step_initialize_iteration(dev_cars[self]);
    dev_cars[self]->step_initialize_iteration();
    COAL_Car_step_accelerate(dev_cars[self]);

...
public:
  ...
  __device__  __noinline__ CellBase *next_step(CellBase *position) {
    // Almost random walk.
    const uint32_t num_outgoing = CLEANPTR( position ,CellBase *)->num_outgoing;
    assert(num_outgoing > 0);

    //f("%p\n",CLEANPTR( position ,CellBase *)->outgoing[this->random_int(0, num_outgoing)]);
    return CLEANPTR( position ,CellBase *)->outgoing[this->random_int(0, num_outgoing)];
  }

...
class Edge : public ChiEdge<EdgeValue> {
	...
	ALL int getVertexIdConcrete() {
	    return vertexId;
	}
	ALL EdgeValue getValueConcrete() {
	    return edgeValue;
	}
	ALL void setValueConcrete(EdgeValue x) {
	    edgeValue = x;
	}
    private:
	EdgeValue edgeValue;
...
class VirtVertex {
	...
	ALL virtual void setOutEdge(VirtVertex<VertexValue, EdgeValue> **vertex, int src, int idx, int vertexId, EdgeValue value) = 0;
    //private:
	//int id;
	//int nInedges;
	//int nOutedges;
...
class ChiVertex : public VirtVertex<VertexValue, EdgeValue> {
	...
	ALL ChiEdge<EdgeValue>* edge(int i) {
	    if (i < nInedges) return inEdgeDataArray[i];
	    else return inEdgeDataArray[i - nInedges];
	}
	...
	ALL void setOutEdge(VirtVertex<VertexValue, EdgeValue> **vertex, int src, int idx, int vertexId, EdgeValue value) {
	...
	//Concrete
	ALL int getIdConcrete() {
	    return id;
	}
	ALL void setIdConcrete(int x) {
	    id = x;
	}
	ALL VertexValue getValueConcrete() {
	    return value;
	}
	ALL void setValueConcrete(VertexValue x) {
	    value = x;
	}
	ALL int numEdgesConcrete() {
	    return nInedges + nOutedges;
	}
	ALL int numInEdgesConcrete() {
	    return nInedges;
	}
	ALL int numOutEdgesConcrete() {
	    return nOutedges;
	}
	ALL ChiEdge<EdgeValue>* edgeConcrete(int i) {
	    if (i < nInedges) return inEdgeDataArray[i];
	    else return inEdgeDataArray[i - nInedges];
	}
	ALL ChiEdge<EdgeValue>* getInEdgeConcrete(int i) {
	    return inEdgeDataArray[i];
	}
	ALL ChiEdge<EdgeValue>* getOutEdgeConcrete(int i) {
	    return outEdgeDataArray[i];
	}
	ALL void setInEdgeConcrete(int idx, int vertexId, EdgeValue value) {
	    inEdgeDataArray[idx] = new Edge<EdgeValue>(vertexId, value);
	}
	ALL void setOutEdgeConcrete(VirtVertex<VertexValue, EdgeValue> **vertex, int src, int idx, int vertexId, EdgeValue value) {
	    //outEdgeDataArray[idx] = new Edge<EdgeValue>(vertexId, value);
	    for (int i = 0; i < vertex[vertexId]->numInEdges(); i++) {
	        if (vertex[vertexId]->getInEdge(i)->getVertexId() == src) {
	            outEdgeDataArray[idx] = vertex[vertexId]->getInEdge(i);

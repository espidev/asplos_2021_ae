...
class VirtVertex {
	...
	ALL virtual void setOutEdge(VirtVertex<float, float> **vertex, int src, int idx, int vertexId, EdgeValue value) = 0;
    //private:
	//int id;
	//int nInedges;
	//int nOutedges;
...
class ChiVertex : public VirtVertex<VertexValue, EdgeValue> {
	...
	ALL void setOutEdge(VirtVertex<float, float> **vertex, int src, int idx, int vertexId, EdgeValue value) {
	...
	ALL void setOutEdgeConcrete(VirtVertex<float, float> **vertex, int src, int idx, int vertexId, EdgeValue value) {
	    //outEdgeDataArray[idx] = new Edge<EdgeValue>(vertexId, value);
	    for (int i = 0; i < vertex[vertexId]->numInEdges(); i++) {
	        if (vertex[vertexId]->getInEdge(i)->getVertexId() == src) {
	            outEdgeDataArray[idx] = vertex[vertexId]->getInEdge(i);

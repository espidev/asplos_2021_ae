...
class Plain: public Object
{
    public:
        ...
        __noinline__ __device__ float3 getNormaleConcrete(float3 P)
        {
            return normalize(make_float3(0.0f,1.0f,0.0f));
        }
        __noinline__ __device__ float intersectionConcrete( Rayon R)
        {
            float res;
            float3 N = normalize(make_float3(0.0f,1.0f,0.0f));
            float m(dot(N,R.u)), d, t;
...
__global__ void initObject( Object** objList, float* A, int n )
{
    ...
    if (i < n) {
        if (i == 0) {
            objList[i] = new Plain(A[i * 8], A[i * 8 + 1], A[i * 8 + 2], A[i * 8 + 3], make_float3(A[i * 8 + 4], A[i * 8 + 5], A[i * 8 + 6]), A[i * 8 + 7]);
        } else {
            objList[i] = new Sphere(A[i * 8], A[i * 8 + 1], A[i * 8 + 2], A[i * 8 + 3], make_float3(A[i * 8 + 4], A[i * 8 + 5], A[i * 8 + 6]), A[i * 8 + 7]);
        }

    }
    //objList[0] = new Plain(0.0f, 1.0f, 1.0f, 1.0f, make_float3(0.0f, -1.5f,-0.0f), 0.5f);
    //objList[1] = new Sphere(1.0f, 0.0f, 0.0f, 1.0f, make_float3(-1.0f, 0.0f,-1.0f), 0.5f);
    //objList[2] = new Sphere(0.0f, 0.0f, 1.0f, 1.0f, make_float3(1.0f, -0.0f, -1.0f), 0.5f);
    //objList[3] = new Sphere(0.0f, 1.0f, 0.0f, 1.0f, make_float3(0.0f, -0.0f, -2.0f), 0.75f);
    //objList[4] = new Sphere(0.0f, 1.0f, 0.0f, 1.0f, make_float3(1.0f, -0.0f, -2.0f), 0.75f);
}

__device__ bool notShadowRay( Object** __restrict__ objList, float3 A, float3 u, int NUM ) {
    float t(0.0f);
    Rayon ray;
	...
	if (j == 0) {
	    t = ((Plain *)objList[j])->intersectionConcrete(ray);
	} else {
	    t = ((Sphere *)objList[j])->intersectionConcrete(ray);
	}
...
__global__  void render(uint * result, Object ** __restrict__ objList, uint imageW, uint imageH, float df, int NUM)
{
    ...
    if( x < imageW && y < imageH )
    {
       ...
       //printf("%d: nRec %d\n", threadIdx.x, nRec);
        for( int i = 0; i < nRec && n == i; i++ ) {
            ...
            for( int j = 0; j < NUM; j++ ) {
                float t;
		if (j == 0) {
		    t = ((Plain *)objList[j])->intersectionConcrete(R);
		} else {
		    t = ((Sphere *)objList[j])->intersectionConcrete(R);
		}
       ...
       //printf("%d: i=%d, t=%e\n", threadIdx.x, i, prof);
            ...
            if( t > 0.0f && t < 10000.0f ) {
                n++;
                float4 color(make_float4(objList[Obj]->R,objList[Obj]->V,objList[Obj]->B,objList[Obj]->A));
                float3 P(R.A+R.u*t), L(normalize(make_float3(10.0f,10.0f,10.0f)-P)), V(normalize(R.A-P));
                float3 N;
		if (Obj == 0) {
		    N = ((Plain *)objList[Obj])->getNormaleConcrete(P);
		} else {
		    N = ((Sphere *)objList[Obj])->getNormaleConcrete(P);
		}

  ...
  if (tid < context->getNumVertices()) {
    if (iteration == 0) {
      if (tid == 0) {
        ...
        int numOutEdge;
        vtable = get_vfunc(vertex[tid], vfun_table);
        temp_coal = vtable[5];
        ...
        for (int i = 0; i < numOutEdge; i++) {
          ...
          vtable2 = get_vfunc(outEdge, vfun_table);
          temp_coal = vtable2[2];
      ...
      for (int i = 0; i < numInEdge; i++) {
        ...
        vtable2 = get_vfunc(inEdge, vfun_table);
        temp_coal = vtable2[1];
...
//   unsigned tree_size = tree_size_g;
  ...
  if (tid < context->getNumVertices()) {
    void **vtable = get_vfunc(vertex[tid], vfun_table);
    temp_coal = vtable[2];
    // printf("%d\n",index[tid]);

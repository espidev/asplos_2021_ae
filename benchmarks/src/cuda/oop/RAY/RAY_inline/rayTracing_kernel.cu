...
class Object {
  public:
    ...
    __inline__ __device__ virtual float intersection(Rayon R) = 0;
    __inline__ __device__ virtual float3 getNormale(float3 P) = 0;
    float R, V, B, A;
    float3 C;  // centre
    float r;   // rayon
};
...
class Plain : public Object {
  public:
    ...
    __inline__ __device__ float3 getNormale(float3 P) {
        return normalize(make_float3(0.0f, 1.0f, 0.0f));
    }
    __inline__ __device__ float intersection(Rayon R) {
        float res;
...
__global__ void initObject(Object** objList, float* A, int n) {
    ...
    // -1.5f,-0.0f), 0.5f); objList[1] = new Sphere(1.0f, 0.0f, 0.0f, 1.0f,
    // make_float3(-1.0f, 0.0f,-1.0f), 0.5f); objList[2] = new Sphere(0.0f,
    // 0.0f, 1.0f, 1.0f, make_float3(1.0f, -0.0f, -1.0f), 0.5f); objList[3] =
    // new Sphere(0.0f, 1.0f, 0.0f, 1.0f, make_float3(0.0f, -0.0f, -2.0f),
    // 0.75f); objList[4] = new Sphere(0.0f, 1.0f, 0.0f, 1.0f, make_float3(1.0f,
    // -0.0f, -2.0f), 0.75f);
}
...
__device__ bool notShadowRay(Object** __restrict__ objList, float3 A, float3 u,
    ...
    for (int j = 0; j < NUM && !t; j++) {
        if (j > 0) {
            t = ((Sphere *)objList[j])->intersection(ray);
        } else {
            t = ((Plain *)objList[j])->intersection(ray);
        }
    ...
    if (x < imageW && y < imageH) {
        ...
        for (int i = 0; i < nRec && n == i; i++) {
            for (int j = 0; j < NUM; j++) {
                float t;
                if (j > 0) {
                    t = ((Sphere *)objList[j])->intersection(R);
                } else {
                    t = ((Plain *)objList[j])->intersection(R);
                }
            ...
            if (t > 0.0f && t < 10000.0f) {
                ...
                float3 N;
                if (Obj > 0) {
                    N = ((Sphere *)objList[Obj])->getNormale(P);
                } else {
                    N = ((Plain *)objList[Obj])->getNormale(P);
                }
